...
async function removeEmoji(message: Message, emoji: string) {
  try {
    ...
    if (reaction && botClient.user) await reaction.users.remove(botClient.user?.id);
  } catch (error) {
...
export async function handleMessageLink(message: Message | CommandInteraction, content: string) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    for (let i = 0; i < totalFiles; i++) {
      if (data.files[i]) {
        files.push(data.files[i] as Exclude<(typeof data.files)[number], undefined>);
      }
...
export async function actionWhenFoundUrl(message: Message) {
  ...
  if (isAllowed && !message.author.bot) {
    if (message.guild) {
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents([btnGetMedia, btnNo]);
...
export async function handleMessageConvertCurrency(message: Message, content: string) {
  const params = content.replace('convert', '').replace(' to ', ' ').trim().split(' ');
  if (params.length === 3) {
    const data = await convertCurrency(params?.[0] ?? '', params?.[1] ?? '', params?.[2] ?? '');
...
export async function convertCurrency(
  ...
  try {
    ...
    const currencyFrom = textFrom.split(' ').slice(1).join(' ').replace('=', '').trim();
    const fadedDigits = $(`${baseElement} > p:nth-child(2) > span.faded-digits`).text().trim();
