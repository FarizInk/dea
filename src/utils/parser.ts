...
export function generateEmbed(response: Response): EmbedBuilder | null {
  ...
  try {
    if (metadata.provider_data === 'fxtwitter') {
      ...
      return new EmbedBuilder()
        .setColor(0xc72784)
        .setTitle(item.owner.full_name === '' ? item.owner.username : item.owner.full_name)
        ...
        .setTimestamp(
          item.taken_at_timestamp ? new Date(parseInt(item.taken_at_timestamp + '000')) : null
        )
      ...
      return new EmbedBuilder()
        .setColor(0xc72784)
        .setTitle(item.user.full_name === '' ? item.user.username : item.user.full_name)
        .setURL(`https://instagram.com/${item.user.username}`)
        .setAuthor({ name: `@${item.user.username}` })
        .setThumbnail(item.user.profile_pic_url)
        .setTimestamp(item.taken_at ? new Date(parseInt(item.taken_at + '000')) : null)
      ...
      return new EmbedBuilder()
        ...
        .setTimestamp(item.createTime ? new Date(parseInt(item.createTime + '000')) : null)
      ...
      return new EmbedBuilder()
        ...
        .setDescription(item.record.text === '' ? null : (item.record.text ?? null))
        .setThumbnail(item.author.avatar ?? null)
        .setTimestamp(item.record.createdAt ? new Date(item.record.createdAt) : null)
